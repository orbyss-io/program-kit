# Releasing Program Kit 0.10.1

This patch corrects the greenfield architecture placement handoff. Architecture owns planned
consumer paths with semantic and ADR provenance; research sees managed renderer options, blocked
workers preserve their primary diagnostic, and failed runs can refresh context without rewriting
workflow state or approvals. See [the recovery guide](greenfield-architecture-recovery.md).

All Program Kit installable components advance together to `0.10.1`. Orbyss Foundation remains
pinned at `0.1.0`; Forms and Localization remain at `0.1.1`. This patch publishes no runtime
component versions.
...
`artifacts/release-validation-0.10.1.log` plus the machine-bound
`artifacts/release-receipt-0.10.1.json` for later inspection and does not run an optional paid
Codex-worker phase.
...
should Program Kit `v0.10.1` be tagged. Legacy `ProgramKit.*` handling is permanently read-only:
