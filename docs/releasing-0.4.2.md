# Releasing Program Kit 0.4.2

Program Kit `0.4.2` is the Spec Kit bundle version. The .NET packages and host image produced by
the same release advance to `0.4.2-preview.1` so the tag never republishes or overwrites the 0.4.1
runtime artifacts.
...
Check that `VERSION` is `0.4.2`, `RUNTIME_VERSION` is `0.4.2-preview.1`, and the release commit is on
the default branch.

## Create and verify the release

```powershell
git tag -a v0.4.2 -m "Program Kit 0.4.2"
git push origin v0.4.2
```

The tag publishes the three Spec Kit ZIPs plus checksums, the `0.4.2-preview.1` NuGet packages, and
the multi-architecture `ghcr.io/orbyss-io/program-kit-host:0.4.2-preview.1` image. Approve protected
environment deployments only after verifying the tag points at the reviewed release commit. Verify
GitHub attestations, NuGet repository metadata, and the GHCR SBOM/provenance before enabling
consumers.
