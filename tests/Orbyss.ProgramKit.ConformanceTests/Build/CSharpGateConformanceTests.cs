...
public sealed class CSharpGateConformanceTests
{
    ...
    [TestMethod]
    public async Task ConcurrentCompilerInvocationsKeepNonceOutsideStableOutputs()
    {
        ...
        try
        {
            ...
            var secondProject = CreateExternalGeneratedProject(
                temporaryDirectory,
                "second");
            var programKitRoot = ConformanceInputs.ProgramKitRoot;
            ...
            foreach (var project in new[] { firstProject, secondProject })
            {
                ...
                receiptPaths.Add(receipts[0]);
                AssertCompilerInvocationReceipt(
                    receiptRoot,
                    receipts[0],
                    nonces);
            }
            ...
            Assert.AreSequenceEqual(
                File.ReadAllBytes(receiptPaths[0]),
                File.ReadAllBytes(receiptPaths[1]));
            AssertMatchingCompiledOutput(firstProject, secondProject, ".dll");
            AssertMatchingCompiledOutput(firstProject, secondProject, ".pdb");
        }
        finally
        {
        ...
        HashSet<string> observedNonces)
    {
        ...
        Assert.AreEqual(
            "ProgramKitCompilerInvocationReceipt.cs",
            fileName);
        var nonce = Path.GetRelativePath(receiptRoot, receipt)
            .Split(
    ...
    private static void AssertMatchingCompiledOutput(
        string firstProject,
        string secondProject,
        string extension)
    {
        var fileName = string.Concat("GeneratedHost", extension);
        var first = Path.Combine(
            Path.GetDirectoryName(firstProject)!,
            "bin",
            "Release",
            "net10.0",
            fileName);
        var second = Path.Combine(
            Path.GetDirectoryName(secondProject)!,
            "bin",
            "Release",
            "net10.0",
            fileName);
