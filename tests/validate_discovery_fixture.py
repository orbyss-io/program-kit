...
CASES = ('household-shopping', 'repair-desk')


def validate(case, session=None):
    CASE = ROOT / 'tests/live/scenarios' / case / 'v1'
    ...
    if session:
        state = json.loads((session / 'session.json').read_text(encoding='utf-8-sig'))
...
def main():
    ...
    parser.add_argument('--case', choices=CASES, default='household-shopping')
    args = parser.parse_args()
    for case in CASES:
        validate(case, args.session if case == args.case else None)
