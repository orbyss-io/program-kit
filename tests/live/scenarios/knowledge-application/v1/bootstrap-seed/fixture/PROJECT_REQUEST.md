...
The public HTTP fixture contract is acceptance/http-contract.json. Use the browser labels in acceptance/browser-contract.json. Bootstrap writes architecture and feasibility evidence only; it must not create product projects, a frontend or product tests. RM01 owns the reservation slice; RM02 reporting stays planned. The disposable local test surface is anonymous; authentication/deployment variants are separate deterministic tests.
