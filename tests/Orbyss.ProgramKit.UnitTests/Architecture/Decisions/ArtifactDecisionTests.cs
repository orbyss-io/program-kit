...
namespace Orbyss.ProgramKit.UnitTests.Architecture.Decisions;
...
public sealed class ArtifactDecisionTests
{
    ...
    public void CompleteNineQuestionDecisionIsValid()
    {
        ...
        ArtifactDecisionValidator sut = new(ownershipResolver);

        var result = sut.Validate(decision);
    ...
    public void EverySupportedArtifactKindHasOneDocumentedOwnershipRule()
    {
        ...
        SupportedArtifactKindOwnershipResolver sut = new();
        var rules = sut.All;

        Assert.HasCount(kinds.Length, rules);
        for (var index = 0; index < kinds.Length; index++)
        {
            var rule = sut.Resolve(kinds[index]);
