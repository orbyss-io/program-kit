...
public sealed class ArchitectureDesignContractTests
{
    ...
    public void FullSyntheticDesignClosesItsDeclaredCrossReferences()
    {
        ...
        SupportedArtifactKindOwnershipResolver ownershipResolver = new();
        ArtifactDecisionValidator artifactDecisionValidator = new(ownershipResolver);
        ArchitectureDesignValidator sut = new(artifactDecisionValidator);

        var result = sut.Validate(design);

        Assert.IsTrue(result.IsValid, Format(result));
        Assert.HasCount(1, design.UnresolvedDecisions);
        Assert.HasCount(1, design.SemanticModels);
        Assert.HasCount(1, design.Operations);
        Assert.HasCount(5, design.Extensions);
        Assert.HasCount(1, design.Configuration);
        Assert.HasCount(1, design.FeatureActivations);
        Assert.HasCount(1, design.RepresentationRelationships);
        Assert.AreSequenceEqual(
            Enum.GetValues<ExtensionKind>(),
            design.Extensions.Select(static extension => extension.Kind),
    ...
    public void DuplicateArtifactDecisionsForOneArtifactFailClosed()
    {
        ...
        var result = sut.Validate(duplicate);
    ...
    public void UndefinedEnumValuesFailClosedAcrossTheArchitectureContract()
    {
        ...
        Assert.IsFalse(result.IsValid);
        Assert.Contains(ArchitectureDiagnosticIds.Pkarc124, diagnosticIds);
        Assert.Contains(ArchitectureDiagnosticIds.Pkarc212, diagnosticIds);
        Assert.Contains(ArchitectureDiagnosticIds.Pkarc331, diagnosticIds);
        Assert.Contains(ArchitectureDiagnosticIds.Pkarc637, diagnosticIds);
        Assert.Contains(ArchitectureDiagnosticIds.Pkarc638, diagnosticIds);
        Assert.Contains(ArchitectureDiagnosticIds.Pkarc639, diagnosticIds);
        Assert.Contains(ArchitectureDiagnosticIds.Pkarc640, diagnosticIds);
    }
