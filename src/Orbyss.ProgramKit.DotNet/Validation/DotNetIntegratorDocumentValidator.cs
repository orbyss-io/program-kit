...
public sealed class DotNetIntegratorDocumentValidator : IDotNetIntegratorDocumentValidator
{
    ...
    public ProgramKitValidationResult Validate(OpenConsoleDocument document)
    {
        ...
        {
            Error(diagnostics, "Open Console must select the exact 1.0.0 schema and frozen token-array parsing conventions.", string.Empty);
        }
        ...
        {
            Error(diagnostics, "Open Console commands, global options, help, and completion must be explicit and valid.", "/commands");
            return ProgramKitValidationResult.From(diagnostics);
        }
        ...
        foreach (var command in document.Commands)
        {
            ...
            if (command.Path.IsDefaultOrEmpty ||
                ...
                !operationKeys.Add(DotNetContractKeys.Exact(command.OperationRevision)))
            {
                Error(diagnostics, "Command paths and operation revisions must be non-empty and unique.", "/commands");
            }

            if (command.Aliases.IsDefault)
            {
                Error(diagnostics, "Command aliases must be initialized token arrays.", "/commands/aliases");
            }
            else
            {
                foreach (var alias in command.Aliases)
                {
                    ...
                    {
                        Error(diagnostics, "Command aliases must be unique non-empty token arrays.", "/commands/aliases");
                    }
                }
            }
            ...
            if (command.ExitCodes.IsDefaultOrEmpty ||
                ...
                command.ExitCodes.Select(static exit => exit.Code).Distinct().Count() != command.ExitCodes.Length)
            {
                Error(diagnostics, "Each command requires an exhaustive unique exit map including success code 0 and invalid-invocation code 2.", "/commands/exitCodes");
            }
        }
        ...
        {
            Error(diagnostics, "Console provenance must bind exactly the projected operations.", "/provenance/operationRevisions");
        }
        ...
        ImmutableArray<ProgramKitDiagnostic>.Builder diagnostics)
    {
        ...
        {
            Error(diagnostics, "Command arguments must be initialized.", "/commands/arguments");
            return;
        }
        ...
        for (var index = 0; index < arguments.Length; index++)
        {
            ...
            if (argument.Position != index ||
                ...
                !ValidRange(argument.Occurrence.Minimum, argument.Occurrence.Maximum) ||
            ...
            {
                Error(diagnostics, "Arguments require contiguous positions, token names, known types, one token per occurrence, valid occurrence, typed defaults, and consistent required state.", "/commands/arguments");
            }
        }
    }
        ...
        ImmutableArray<ProgramKitDiagnostic>.Builder diagnostics)
    {
        ...
        {
            Error(diagnostics, "Command options must be initialized.", "/commands/options");
            return;
        }
        ...
        foreach (var option in options)
        {
            var valid = IsToken(option.LongName) &&
                        ...
                        ValidRange(option.ValueArity.Minimum, option.ValueArity.Maximum) &&
                        ValidRange(option.Occurrence.Minimum, option.Occurrence.Maximum) &&
                        option.Occurrence.Maximum > 0 &&
                        option.Required == (option.Occurrence.Minimum > 0) &&
                        option.Aliases.IsDefault is false &&
                        option.Conflicts.IsDefault is false &&
                        option.Prerequisites.IsDefault is false &&
            ...
            if (!valid)
            {
                Error(diagnostics, "Options require unique names, known types, and valid arity/occurrence.", "/commands/options");
            }
        }
        ...
        foreach (var option in options)
        {
            ...
            {
                Error(diagnostics, "Option conflicts and prerequisites must reference distinct canonical options in the same command.", "/commands/options");
            }
        }
    }
