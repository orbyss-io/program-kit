...
namespace ObservatoryScheduling.Console.Composition;
...
internal static class Program
{
    private static async Task Main(string[] args)
    {
        if (args is ["--help"])
        {
            global::System.Console.Out.WriteLine(
                "schedule --target <name> --earliest <instant> --latest <instant>");
            return;
        }
        ...
        builder.WebHost.ConfigureKestrel(
            static options => options.ListenLocalhost(43102));
        builder.AddShells(
            static cshells =>
                cshells.AddShell(
                    "observatory-console",
    ...
    private static void ConfigureFixtureServices(IServiceCollection services)
    {
        ...
        services.AddFirstAvailableTaskActivation("observatory-console");
    ...
    private static void MapHealth(WebApplication app)
    {
        ...
        _ = app.MapHealthChecks("/health/ready", options);
    }
}
