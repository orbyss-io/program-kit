...
- [X] T106 CRITICAL Establish one versioned canonical session-integration definition artifact, load and validate its complete governed contract through `SessionIntegrationDefinitionLoader`, replace placeholder fingerprints with exact content identities, and add round-trip, drift, and projection-binding tests per FR-007, FR-012, FR-013, Constitution III, and Constitution IV
- [X] T107 CRITICAL Make the embedded Codex provider manifest the single validated runtime source for provider, adapter, definition, conformance, surface, tested-version, diagnostic, support, and projection identities; reject placeholder or divergent identities and prove manifest/runtime equality per FR-012 through FR-014 and FR-034 through FR-036
- [X] T108 CRITICAL Verify the selected CLI release before candidate admission by resolving the exact workspace-local executable and checking the CLI identity schema/profile, package identity and digest evidence, command, executable digest, reported version, and runtime profile against observed evidence; add unavailable and mismatch fixtures for every field per FR-002, FR-018, FR-020, SC-007, US1/AC1, and the executable-mismatch edge case
- [X] T109 CRITICAL Invoke the declared provider conformance profile during production resolution and admission, and fail closed on support, scope, operation, ownership, definition, diagnostic, projection, disclosure, authority, or normalization loss instead of relying on test-only conformance per FR-014, FR-018, FR-020, FR-038, US4/AC2, and Constitution VI
- [X] T110 CRITICAL Harden installation inspection and verification to validate the record schema and digest, admission/publication evidence, and exact binding to the current CLI, canonical definition, provider, adapter, conformance profile, workspace, and projection set; classify corrupt, stale, incompatible, partial, idempotent, reload-required, and fresh-session states without mutation per FR-013 and FR-020 through FR-023, SC-002, SC-007, and US1/AC1–AC4
- [X] T111 Complete the packaged-CLI negative matrix for malformed, ambiguous, unsupported, unavailable CLI/executable, missing or incompatible provider, collision, interruption, stale state, drift, and missing authority; assert distinct diagnostic identities, dispositions, effect states, and byte preservation per FR-039 through FR-043, SC-004, and US3/AC1–AC4

## Phase 10: Convergence

**Purpose**: Repair only the rejected-review and `main`-merge invalidations, then
return Feature 002 to protected CI and explicitly human-owned validation without
rewriting T001–T111 or repeating unaffected local proof.

- [X] T112 CRITICAL Make every production session diagnostic satisfy the merged public diagnostic contract with typed catalog disposition, safe expected/observed values, bounded non-authorizing remediation, and non-empty exact catalog evidence; make the every-ID golden test validate each projected result against `program-kit.operation-result/v1` before rebinding its digest (contradicts) | Refs: FR-032, FR-039, FR-040, FR-042, Constitution VII | Proof: focused `SessionDiagnosticCatalogContractTests`, `SessionNegativeResultGoldenTests`, and schema validation | Tier: story | Done: every retained `PKSES`/`PKCDX` diagnostic projects a schema-valid safe result with exact evidence and the focused contract filter passes
- [X] T113 Replace the obsolete live-review consumer inputs with one exact current Feature 001 factory request/grant/review/revocation closure and add a read-only preflight that fails before provider launch on a missing, stale, mismatched, or zero-digest seed (missing) | Refs: FR-002, FR-027, FR-028, FR-029, SC-003, SC-005, product-review conditions 1–2 | Proof: focused seed/preflight contract or acceptance cases covering exact, missing-review, missing-revocation, stale-binding, and zero-digest inputs | Tier: story | Done: the review packet uses current `construct.json` plus exact authority closure and every invalid seed stops without launching Codex
- [X] T114 Strengthen canonical and Codex-projected guidance so a session consumes typed missing-input/continuation fields, asks only the bounded human question, treats conversation as non-authority, uses an explicitly selected exact grant artifact, and evaluates every admitted construction; add deterministic positive and negative guidance journeys before changing projection bytes (partial) | Refs: FR-010, FR-025 through FR-032, SC-003, SC-005, US2/AC1–AC4 | Proof: focused guidance contract tests and `HumanLedSessionWorkflowAcceptanceTests` for missing semantic input, incomplete authority, exact continuation, successful evaluation, and declined authority | Tier: story | Done: the public guidance and focused journey proof encode an unambiguous explain–clarify–authorize–construct–evaluate sequence without invented authority
- [X] T115 Bind the opt-in ten-session harness and its bounded evidence to the exact current CLI, projection, canonical definition, scenario, and review-seed identities; record reviewer-attested operation order and typed final outcome/effect/disposition without prompts, responses, transcripts, raw provider output, credentials, or paths; validate the evidence schema and preserve the rejected record as historical evidence (partial) | Refs: FR-024, FR-032, FR-042, SC-003, SC-005, SC-009 | Proof: review-evidence schema/contract tests plus launcher fail-closed tests that do not start a provider | Tier: story | Done: malformed or unbound evidence cannot report `review-ready`, valid evidence represents all SC-003/SC-005 attestations, and the old 8/10 record is not overwritten
- [X] T116 Refresh only merge-invalidated session golden and distribution evidence at the exact implementation candidate, then run one isolated pre-PR restore/build, complete unit/contract, changed-file formatting, SpecKit integrity, canonical text, and diff-hygiene gate without running the full local acceptance/conformance/platform matrix (partial) | Refs: plan: Verification Strategy, Constitution IV, Constitution IX | Proof: `eng/Invoke-Verification.ps1 -Mode PrePr` plus exact evidence regeneration/staleness checks | Tier: pre-pr | Done: the candidate is clean, generated evidence is current, all unit/contract proof passes, and no full local CI-equivalent gate was substituted
- [X] T117 Obtain authoritative protected Windows/Linux CI proof for the exact pushed candidate, including full acceptance, conformance, deterministic evidence, disclosure, runtime isolation, and supported-platform workspace matrix, without duplicating that full gate locally (missing) | Refs: SC-001, SC-002, SC-004, SC-006 through SC-010, plan: CI tier, Constitution IX | Proof: required protected GitHub checks for the exact candidate | Tier: ci | Done: both required platform jobs pass for the same candidate and their bounded evidence identities are recorded
- [X] T118 Execute a new explicitly authorized ten-consecutive-fresh-session review against the exact CI-green packet and record a separate bounded evidence document; do not overwrite or reinterpret the rejected 8/10 run (missing) | Refs: FR-024 through FR-032, SC-003, SC-005, product-review condition 3 | Proof: human-operated `eng/Invoke-CodexSessionReview.ps1` with exactly 10 trials | Tier: human | Done: `reviews/codex-session-review-remediated.json` is schema-valid, bound to the exact `16e08c8` review packet, and truthfully reports 10/10 with status `review-ready`
- [X] T119 Obtain a new independent human product-review decision over the exact CI and fresh-session evidence, recording reviewer identity, decision, limitations, invalidation set, and timestamp without treating automation or agent judgment as approval (missing) | Refs: SC-003, SC-005, product-review condition 4, Constitution I, Constitution IX | Proof: named independent decision in `reviews/product-review.md` | Tier: human | Done: reviewer `joey-orbyss` explicitly approved the bounded remediated candidate with limitations and invalidation set; release, publication, and merge remain separate
- [X] T120 Reconcile every FR/SC and the Phase 10 ledger against the exact executable and human evidence, preserving superseded/rejected provenance and leaving Feature 002 incomplete unless all mandatory gates—including T117–T119—are satisfied (missing) | Refs: all FR-001 through FR-046, SC-001 through SC-010, Constitution IX | Proof: final requirement/evidence table plus follow-up `$speckit-analyze` with no unresolved CRITICAL/HIGH finding | Tier: human | Done: all 46 FRs and 10 SCs reconcile to exact automated or human evidence, rejected provenance remains visible, the follow-up analysis found 100% requirement coverage and no CRITICAL/HIGH finding, and bounded product acceptance rests only on the named independent decision

## Phase 11: Human-Review Usability Convergence

**Purpose**: Repair only the authority-prompt and launcher-usability failures
observed while starting T118, refresh the invalidated executable proof once, and
return the exact repaired candidate to the still-pending human review. T118–T120
remain pending and must not be inferred from this automated repair.

- [X] T121 HIGH Require canonical and Codex-projected session guidance to read the construct request's existing authority binding, name its exact logical path or identity when asking the human to select it, and stop instead of asking the human to discover or guess authority (partial) | Refs: FR-025 through FR-029, SC-003, SC-005, US2/AC2–AC3, Constitution I | Proof: focused `SessionGuidanceContractTests` and `HumanLedSessionWorkflowAcceptanceTests` covering exact naming and vague-request rejection | Tier: story | Done: the session itself names the request-bound grant before selection and no instruction shifts grant discovery to the reviewer
- [X] T122 HIGH Extend the bounded live-review protocol and `codex-session-review/v2` evidence so the reviewer separately attests that the session named the exact existing request-bound grant; count a vague authority request as a failed trial without recording prompts, responses, transcripts, or local absolute paths (missing) | Refs: FR-027, FR-029, FR-032, FR-042, SC-003, SC-005, SC-009 | Proof: review schema/contract tests plus launcher prompt and seed-binding tests that do not start Codex | Tier: story | Done: `exactGrantNamed` is required and true for every passing trial, while evidence remains disclosure-safe and bound to the canonical logical grant path
- [X] T123 Make the opt-in human-review launcher fail early with actionable PowerShell 7 guidance and resolve Codex from either an explicit executable path or supported discovery locations while preserving exact version checks and never mutating global or session PATH (partial) | Refs: FR-024, SC-003, plan: Technical Context and live-review protocol | Proof: focused non-launching script contract tests for unsupported PowerShell, missing/invalid explicit executable, fallback discovery, and exact-version refusal | Tier: story | Done: unsupported shells and missing Codex report the exact corrective command or parameter before any provider process can start
- [X] T124 Refresh only guidance/projection/generated evidence invalidated by T121–T123, run the affected unit/contract/acceptance filters, and run one `eng/Invoke-Verification.ps1 -Mode PrePr` gate without launching Codex or duplicating the full platform matrix locally (partial) | Refs: FR-042, SC-009, plan: Verification Strategy, Constitution IV and IX | Proof: focused filters, evidence staleness checks, and the fast pre-PR gate | Tier: pre-pr | Done: generated identities match the repaired candidate, focused regressions pass, and the local gate remains proportional
- [X] T125 Obtain authoritative protected Windows/Linux CI proof for the exact pushed T121–T124 candidate and record its check identities before preparing a fresh isolated T118 review packet; do not rerun the equivalent full gate locally (missing) | Refs: SC-001, SC-002, SC-004, SC-006 through SC-010, plan: CI tier, Constitution IX | Proof: required protected GitHub checks for the exact repaired candidate | Tier: ci | Done: both required platform jobs pass for one exact candidate and T118 starts only from a newly prepared packet bound to that candidate

## Phase 12: Live Construction Recovery Convergence

**Purpose**: Repair only the exact dependency-closure, retry, and diagnostic gaps
observed in the aborted first T118 trial. The aborted trial is discovery input,
not acceptance evidence; T118–T120 remain pending.

- [X] T126 CRITICAL Roll back only the unsealed candidate created by a failed provider invocation before returning a retry disposition, report the resulting effect state truthfully, and prove that retrying the same still-current authorized construction cannot be converted into `PKWSP0003` by residue from the failed attempt (contradicts) | Refs: FR-031, FR-039, US3/AC4, Constitution V and VII | Proof: focused provider-failure acceptance test covering thrown and returned provider failures, exact candidate cleanup, no live/admission effects, and a successful second invocation | Tier: story | Done: a provider failure leaves no incomplete candidate owned by that invocation and the typed retry is operationally possible
- [X] T127 HIGH Distinguish an unavailable declared dependency mirror before candidate creation from a genuine executed `.NET` tool failure, and give genuine retryable provider failures safe bounded observation plus a complete retry request instead of generic `help` guidance (partial) | Refs: FR-031, FR-039 through FR-041, US3/AC4, Constitution VII | Proof: focused diagnostic/operation-result tests for missing mirror `PKEXT0002` with no effects and real `PKDOT0006` with safe actionable retry data | Tier: story | Done: pre-tool unavailability and post-launch tool failure are independently distinguishable and every returned next action is executable only as a separately revalidated invocation
- [X] T128 HIGH Bind the exact governed dependency mirror into the Codex review-seed contract and packet, copy it during seed initialization, and make read-only preflight reject a missing, changed, incomplete, extra, or stale mirror before resolving or launching Codex (partial) | Refs: FR-002, FR-029, SC-003, T113 completion claim, Constitution IV and IX | Proof: focused review-seed contract tests plus one disposable initialized-packet construct/evaluate smoke without launching Codex | Tier: story | Done: every admitted T118 trial begins with the exact complete offline construction closure and the reproduced missing-mirror packet cannot pass preflight
