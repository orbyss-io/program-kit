...
- [X] T010 [P] Define failing adapter diagnostic aggregation and safe-fallback tests in `tests/ProgramKit.ContractTests/SpecKitAdapterDiagnosticTests.cs` | Refs: FR-027, FR-029, NEG-008, CON-VII | Proof: focused contract test | Tier: story | Done: all 12 public adapter IDs have exact typed catalog definitions; real production-boundary reachability is proved by T075 rather than factory synthesis
...
- [X] T066 [P] [US6] Add extension package lifecycle and ownership tests in `tests/ProgramKit.AcceptanceTests/SpecKitAdapterLifecycleAcceptanceTests.cs` | Refs: FR-008, FR-016, SC-005, SC-006, NEG-009, CON-V | Proof: staged extension install/update/disable/re-enable/remove harness | Tier: story | Done: the real Spec Kit 0.15.1 CLI installs from an exact local catalog, updates 0.0.9 to the packaged 0.1.0 release, preserves exact consumer/Program Kit artifacts, retains 0.1.0 after an incompatible 0.2.0 attempt, visibly revalidates after re-enable, and removes through `--keep-config`
- [X] T067 [P] [US6] Add cleanup ownership tests in `tests/ProgramKit.UnitTests/SpecKitAdapterCleanupTests.cs` | Refs: FR-016, FR-028, NEG-007, NEG-009 | Proof: focused exact/drifted/differently-owned matrix | Tier: story | Done: exact candidates become removed tombstones and bytes are deleted; retained, differently-owned, unknown, handoff, and Program Kit state remain, while drift, escape, and case collision refuse atomically

### Implementation for User Story 6

- [X] T068 [US6] Implement explicit cleanup in `src/ProgramKit.SpecKitAdapter/Commands/CleanupCommand.cs` and `src/ProgramKit.SpecKitAdapter/Publication/AdapterCleanupService.cs` | Refs: FR-016, FR-028, CON-V | Proof: T067 | Tier: edit | Done: cleanup updates trust first, supports interruption-safe retry, removes only unchanged regenerable adapter definitions, and preserves every retained, consumer-owned, Program Kit-owned, unknown, or drifted artifact
- [X] T069 [US6] Complete versioned extension update/disable/re-enable/remove ownership declarations in `extensions/orbyss-program-kit-adapter/extension.yml`, `extensions/orbyss-program-kit-adapter/package-manifest.json`, and `eng/Pack-SpecKitAdapter.ps1` | Refs: FR-008, FR-016, NEG-009 | Proof: T066 | Tier: edit | Done: the package distinguishes release template from top-level consumer config, declares supported lifecycle ownership, publishes a byte closure through staged rollback-safe activation, and uses Spec Kit's mandatory exact extension command namespace
- [X] T070 [US6] Add manifest-aware Spec Kit upgrade acceptance to `tests/ProgramKit.AcceptanceTests/SpecKitAdapterLifecycleAcceptanceTests.cs` using `eng/Invoke-SpecKitUpgrade.ps1` | Refs: FR-016, SC-005, NEG-009, CON-WF | Proof: clean compatible upgrade without force | Tier: story | Done: a clean real `integration upgrade` through the production script preserves exact extension registration, consumer config, handoff/review, source, Program Kit state/product/result bytes, and invokes no force option
- [X] T071 [US6] Add platform-sensitive extension lifecycle jobs to `.github/workflows/vertical-slice.yml` | Refs: FR-008, FR-016, SC-005, SC-006 | Proof: Windows/Linux packaged matrix definition | Tier: ci | Done: the exact Spec Kit 0.15.1 CLI and Python fixture server are installed only in the Windows/Linux platform job; the lifecycle acceptance is excluded from neutral core proof and included once per supported OS
- [X] T072 [US6] Run the focused User Story 6 verification set through `eng/Invoke-Verification.ps1` | Refs: US6, FR-008, FR-016, FR-028 | Proof: cleanup and single-platform lifecycle filters | Tier: story | Done: 2 unit, 5 contract, and 1 real lifecycle acceptance test pass in 16 seconds with zero build warnings/errors; unrelated acceptance, evidence generation, and the protected second-OS execution remain CI-owned
...
- [X] T073 [P] Add a second semantically distinct clean factory fixture in `tests/Fixtures/SpecKitAdapter/Inventory.Health/` | Refs: FR-032, SC-001, CON-IX | Proof: schema/canonical fixture validation | Tier: story | Done: Inventory Health has a distinct feature, contract, route, namespace, scoped custom behavior, canonical handoff/review, definitions, and requests; the two-fixture contract test passes
- [X] T074 Complete the two clean natural-language-to-evaluate package journeys in `eng/Invoke-SpecKitAdapterQuickstart.ps1` and `tests/ProgramKit.AcceptanceTests/SpecKitAdapterQuickstartAcceptanceTests.cs` | Refs: FR-032, SC-001, SC-007, CON-IX | Proof: two separately initialized packaged consumers | Tier: ci | Done: both declared journeys start without Program Kit semantic state, initialize/select through the public CLI, stage reviewed intent explicitly, use production authority, and verify build/test/runtime isolation; one Reference Status local smoke passed in 18 seconds and the complete two-platform execution remains T081-owned
- [X] T075 Reconcile all nine negative groups into one executable matrix in `tests/ProgramKit.AcceptanceTests/SpecKitAdapterNegativeMatrixAcceptanceTests.cs` | Refs: FR-032, SC-004, NEG-001 through NEG-009, CON-VII | Proof: every retained case invokes its production boundary and asserts exact outcome/stage/effect/disposition/ID/safe expected-observed/evidence/no unauthorized effects | Tier: ci | Done: all 12 public adapter IDs are now typed and production-reachable across the nine groups; the focused two-test matrix passes in 9 seconds without catalog synthesis or product effects
- [X] T076 [P] Extend distribution/package evidence generation in `eng/GenerateDistributionEvidence.cs`, `eng/Generate-DistributionEvidence.ps1`, and `artifacts/evidence/` inputs | Refs: FR-031, FR-032, FR-035, CON-IV, CON-IX | Proof: generated evidence/schema/staleness contract tests | Tier: ci | Done: generated canonical evidence binds exact release/archive closure, compatibility, public schemas, diagnostic catalog, provider support, dependency lock, and semantic invalidation sets; the focused evidence contract passes
- [X] T077 [P] Add complete tool/extension package inspection in `tests/ProgramKit.ContractTests/SpecKitAdapterPackageClosureTests.cs` | Refs: FR-008, FR-009, FR-031, FR-034, FR-035, FR-037 | Proof: packed nupkg/archive contents, dependencies, executable, schemas, licenses, and forbidden-file inspection | Tier: ci | Done: deterministic archive and NuGet inspection pass in one second; public contracts and license declaration are inspectable and no source/debug/test/self-hosting/kernel/provider/session/dynamic-provider coupling ships with the adapter
- [X] T078 Update product guidance and honest limitations in `README.md`, `extensions/orbyss-program-kit-adapter/README.md`, and `specs/003-speckit-adapter/quickstart.md` | Refs: FR-031, FR-032, SC-008, CON-IX | Proof: documentation claim-to-evidence review | Tier: pre-pr | Done: install, exact selection, no-code applicability, reviewed handoff, separate authority, ownership, lifecycle, verification tiers, and deferred capabilities now match the executable product
- [X] T079 Reconcile requirement, constitution, negative, and proof ownership against implementation in `specs/003-speckit-adapter/tasks.md` | Refs: FR-001 through FR-037, SC-001 through SC-011, CON-I through CON-WF, NEG-001 through NEG-009 | Proof: `$speckit-analyze` reports no CRITICAL/HIGH coverage or inconsistency finding | Tier: pre-pr | Done: analysis found 37/37 functional requirements, 11/11 success criteria, nine/nine negative groups, and every constitutional row owned by production implementation and non-synthetic proof, with final CI and human obligations still explicitly pending
- [X] T080 Run the repository pre-PR integration pass through `eng/Invoke-Verification.ps1` | Refs: FR-033, SC-009, CON-WF | Proof: `./eng/Invoke-Verification.ps1 -Mode PrePr` | Tier: pre-pr | Done: Spec Kit/canonical integrity, locked restore, all affected release-equivalent builds with zero warnings/errors, 59 unit tests, 128 contract tests, and the one staged adapter bootstrap smoke pass; full acceptance, conformance, evidence, and Windows/Linux proof remain CI-owned
