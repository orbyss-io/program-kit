...
1. Download the Windows command initializer:
   ...
   ```powershell
   Invoke-WebRequest `
     https://github.com/orbyss-io/program-kit/releases/download/v0.9.5/Initialize-ProgramKit-0.9.5.cmd `
...
1. Download the Bash initializer:
   ...
   ```bash
   curl -fL \
     https://github.com/orbyss-io/program-kit/releases/download/v0.9.5/Initialize-ProgramKit-0.9.5.sh \
...
```powershell
python C:\path\to\program-kit-0.9.5\scripts\upgrade_program_kit.py `
  --release-root C:\path\to\program-kit-0.9.5 `
  --target . `
  --integration codex
```
...
[`docs/releasing-0.9.5.md`](docs/releasing-0.9.5.md).
...
```powershell
gh attestation verify program-kit-0.9.5.zip --repo orbyss-io/program-kit
Get-FileHash program-kit-0.9.5.zip -Algorithm SHA256
```
