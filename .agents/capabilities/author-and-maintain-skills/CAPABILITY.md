...
- One canonical provider-neutral definition at `.agents/capabilities/<capability-id>/CAPABILITY.md` when a capability is registered.
...
The canonical capability definition is the source of truth for procedure and boundaries. The capability index owns registration and availability. Provider wrappers own only provider-specific trigger metadata and loading instructions.
...
5. Add or update provider wrappers as thin adapters only. For example, a Codex wrapper may use Codex skill front matter for triggering, then instruct Codex to load `.agents/capabilities/<capability-id>/CAPABILITY.md` completely before acting.
...
Each registered provider wrapper must point to exactly one canonical definition. The wrapper may contain provider-required metadata and concise loading instructions, but must not copy the canonical purpose, procedure, boundaries, or verification rules.

## Drift check

For every wrapper update:

- Verify the wrapper path links to the exact canonical definition path.
...
- Migrate or remove provider wrappers so stale registrations do not remain.
