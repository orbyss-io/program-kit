...
- The canonical `.agents/capabilities/INDEX.md`.
...
availability comes only from `.agents/capabilities/INDEX.md`; refresh its digest
immediately before emitting a durable result. Provider wrappers are registration
adapters, not procedure or availability sources.
...
The Codex wrapper at
`.codex/skills/develop-software/SKILL.md` may contain only provider registration
metadata and an instruction to load this file completely. Verify that it points
to this exact canonical path, does not copy this procedure, and that the index
links to both files.
